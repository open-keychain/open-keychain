...
public class HkpKeyserverClient implements KeyserverClient {
    ...
    private static final Pattern UID_LINE = Pattern
            .compile("(?<uid>uid:" +              // group 1
                            "(?<uidID>[^:\n]*)" +       // group 2
                            "(:(?<uidCreate>[0-9]*)" +  // group 4
                            "(:(?<uidExpire>[0-9]*)" +  // group 6
                            "(:(?<uidFlags>((?=(r(?!(.?r))|d(?!(.?d))|e(?!(.?e))))[rde]){0,3})" + // group 8
                            ")?)?)?\n)",
                    Pattern.CASE_INSENSITIVE);
    ...
    private static final Pattern PUB_KEY_LINE = Pattern
            .compile( "(?<pub>pub:" +                 // group 1
                            ...
                            "(:(?<pubAlgo>[0-9]*)" +        // group 4
                            "(:(?<pubKeyLen>[0-9]*)" +      // group 6
                            "(:(?<pubCreate>[0-9]*)" +      // group 8
                            "(:(?<pubExpire>[0-9]*)" +      // group 10
                            "(:(?<pubFlags>(?:(?=(?:r(?!(.?r))|d(?!(.?d))|e(?!(.?e))))[rde]){0,3})" + // group 12
                            ")?)?)?)?)?\n)"// pub line
                            + "(?<pubUids>" + UID_LINE.pattern() + // group 16
    ...
    public ArrayList<ImportKeysListEntry> search(String query, ParcelableProxy proxy)
        ...
        while (matcher.find()) {
            ...
            try {
                int bitSize = -1;
                if (!matcher.group(6).isEmpty()){   // empty fields are allowed
                    bitSize = Integer.parseInt(matcher.group(6));
                    entry.setBitStrength(bitSize);
                }

                if (!matcher.group(4).isEmpty()){   // empty fields are allowed
                    int algorithmId = Integer.decode(matcher.group(4));
                    entry.setAlgorithm(KeyFormattingUtils
                            .getAlgorithmInfo(algorithmId, bitSize, null));
                }

                if (!matcher.group(8).isEmpty()) {  // empty fields are allowed
                    long creationDate = Long.parseLong(matcher.group(8));
            ...
            try {
                entry.setRevoked(matcher.group(12).contains("r"));
                boolean expired = matcher.group(12).contains("e");

                // It may be expired even without flag, thus check expiration date
                String expiration;
                if (!expired && !(expiration = matcher.group(10)).isEmpty()) {
            ...
            final String uidLines = matcher.group(16);
